...
export class ReclamtionService {
  private baseUrl = 'http://localhost:8089/pi_cloud/reclamations';
...
getByPriority(priority: string): Observable<Reclamation[]> {
  ...
  analyzePriority(description: string): Observable<string> {
    return this.http.post<string>('http://localhost:8089/pi_cloud/reclamations/analyze-priority', description, {
